-- Streak restore expires after the calendar day the streak was lost.
-- Also stop re-notifying when the streak is already at 0.
...
BEGIN
  ...
  WHERE relationship_id = p_relationship_id
    ...
    AND restorable_lost_at < v_today;
END;
$$;
...
BEGIN
  ...
  IF v_lost_at IS NULL OR v_lost_at < v_today THEN
...
BEGIN
  ...
  v_can_restore := v_effective = 0
    ...
    AND v_row.restorable_lost_at >= v_today;
